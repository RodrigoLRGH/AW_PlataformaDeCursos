...
function ThreadDetailPage() {
  ...
  const onSubmit = async (data: ReplyForm) => {
    ...
    try {
      ...
      setError("root", {
        message: error.response?.data?.message || "Error al responder",
      });
    }
  };

  if (loading) return <p className="text-center py-12 text-2xl">Cargando...</p>;

  if (!thread)
    return <p className="text-center py-12 text-2xl">Hilo no encontrado</p>;
                ...
                {thread.replies?.map((reply) => (
                  <Card key={reply.id}>
                    ...
                    <CardContent>
                      <div className="flex justify-start items-start">
                        <div>
                          <p className="text-md">{reply.body}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
