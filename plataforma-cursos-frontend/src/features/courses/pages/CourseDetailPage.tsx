...
function CourseDetailPage() {
  ...
  useEffect(() => {
    ...
    enrollmentService
      .checkEnrollment(Number(id))
      .then(setIsEnrolled)
      .catch(() => setIsEnrolled(false));

    lessonService
      .getByCourse(Number(id))
      .then(setLessons)
      .catch(() => setLessons([]));
  }, [id]);
  ...
  const handleGenerateCertificate = async () => {
    ...
    try {
      ...
      alert(
        `Certificado generado exitosamente: Codigo: ${cert.certificateCode}`,
      );
    } catch (error) {
      ...
      <div className="min-h-screen bg-fondo">
        ...
        <div className="max-w-4xl mx-auto px-8 py-10 space-y-6">
          ...
          <div className="flex justify-between items-start flex-wrap gap-4">
            <div>
              <h2 className="text-3xl text-primario font-bold mb-2">
                {course.title}
              </h2>
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-primario mb-2">
                {Number(course.price) === 0
                  ? "Gratis"
                  : `$${Number(course.price).toFixed(2)} MXN`}
              </p>
              {isEnrolled ? (
            ...
            <CardContent>
              {lessons.length === 0 ? (
                <p className="text-secundario">
                  No hay lecciones creadas en este curso.
                </p>
              ) : (
          ...
          <Card>
            <CardHeader>
              <CardTitle>Exámenes ({exams.length})</CardTitle>
            </CardHeader>
            <CardContent>
              {exams.length === 0 ? (
                <p className="text-secundario">
                  No hay examenes creados en este curso.
                </p>
              ) : (
                <div className="space-y-2">
                  {exams.map((exam) => (
                    ...
                    >
                      ...
                      {isEnrolled && (
                        <Button
                          size="sm"
                          onClick={() =>
                            navigate(`/courses/${id}/exam/${exam.id}`)
                          }
                        >
                          Tomar examen
                        </Button>
